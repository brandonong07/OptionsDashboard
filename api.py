...
def main():
    ...
    if response is not None and response.ok:
        data = response.json()
        call_prices = list(data["callExpDateMap"]["2026-05-26:0"].keys())
        put_prices = list(data["putExpDateMap"]["2026-05-26:0"].keys())
        ...
        for strike in put_prices:
            opt = option.Option(strike, date(2026, 5, 26), "PUT", data, rate)
            adv = opt.getAdvancedGreeks()
            optionsLadder.append({
                "Direction": opt.getOptionType(),
                "Strike": float(opt.getStrike()),
